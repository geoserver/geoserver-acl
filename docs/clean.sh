...
# Clean build script for GeoServer ACL Documentation
# This script removes all generated files and build artifacts

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "🧹 Cleaning GeoServer ACL Documentation"
echo "======================================="
...
if [ -f "src/api/openapi.yaml" ]; then
    echo "🗑️  Removing src/api/openapi.yaml..."
    rm -f src/api/openapi.yaml
    echo "✅ OpenAPI spec removed"
else
    echo "ℹ️  No OpenAPI spec files to remove"
fi
